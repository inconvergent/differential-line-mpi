...
cdef class Segments:
  ...
  def __init__(self, long nmax, double zonewidth, long procs, long nodes):
    """
    ...
    self.comm = mpi_c.MPI_COMM_WORLD

    mpi_c.MPI_Comm_rank(self.comm, &self.rank)
    ...
    if self.rank == 0:
      ...
      print('nodes: {:d}'.format(self.nodes))
      print('procs: {:d}'.format(self.procs))
      print('zones: {:d}'.format(self.nz))
